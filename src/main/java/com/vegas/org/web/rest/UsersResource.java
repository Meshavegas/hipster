...
/**
 * REST controller for managing {@link com.vegas.org.domain.Users}.
 */
@RestController
@RequestMapping("/api")
public class UsersResource {
     ...
     *         body the new users, or with status {@code 400 (Bad Request)} if the
     *         users has already an ID.
     ...
     *         or with status {@code 500 (Internal Server Error)} if the users
     *         couldn't be updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PutMapping("/users/{id}")
    ...
    /**
     * {@code PATCH  /users/:id} : Partial updates given fields of an existing
     * users, field will ignore if it is null
     *
     * @param id    the id of the users to save.
     * @param users the users to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
     ...
     */
    @GetMapping("/userss")
    public ResponseEntity<List<Users>> getAllUserss(@org.springdoc.api.annotations.ParameterObject Pageable pageable) {
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
     *         the users, or with status {@code 404 (Not Found)}.
     */
    @GetMapping("/users/{id}")
